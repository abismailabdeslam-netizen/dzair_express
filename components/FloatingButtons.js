...
export default function FloatingButtons() {
  ...
  useEffect(() => {
    supabase.from('settings').select('*').then(({ data }) => {
      if (data) {
        data.forEach(r => {
          if (r.key === 'whatsapp_number')    setWhatsapp(r.value)
          if (r.key === 'messenger_username') setMessenger(r.value)
        })
      }
    })
  }, [])
  ...
  return (
    <div style={{ position: 'fixed', bottom: '24px', left: '16px', display: 'flex', flexDirection: 'column', gap: '12px', zIndex: 999 }}>
      ...
      {/* Messenger — الرابط الصحيح هو m.me/اسم_الصفحة */}
      {messenger && (
        <a href={`https://m.me/${messenger.replace('@', '').trim()}`}
          target="_blank" rel="noopener noreferrer"
          style={{ ...btn, background: 'linear-gradient(135deg, #0099FF, #a033ff)' }}
          onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.1)'}
          onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>
