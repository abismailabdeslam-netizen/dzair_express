...
export default function ProductClientSection({ product, settings, disc }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // نحاول أولاً بكل الأعمدة الجديدة
      let { error: err } = await client.from('orders').insert({
      ...
      // إذا فشل بسبب أعمدة غير موجودة نحاول بالأعمدة الأساسية فقط
      if (err && (err.code === '42703' || err.message?.includes('column'))) {
        const { error: err2 } = await client.from('orders').insert({
          product_id: product.id, product_name: product.name,
          first_name: fname, last_name: '', phone, wilaya,
          color: colorName, size: selectedSize,
          quantity: qty, total_price: grandTotal,
          status: 'pending'
        })
      ...
      setError('حدث خطأ في الإرسال: ' + (e.message || 'تحقق من الاتصال'))
    }
